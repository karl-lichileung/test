...
public class MainAppList {
	...
	// Global Variables
	static Logger logger = Logger.getLogger(MainAppList.class.getName()); //log4j	
	
	final String solution_provider = PropertyLoader.getConfig("applist.solution.provider"); // Configuration parameters //	
	static java.net.URL applistxmlurl = null; // remote applist
	static java.net.URL lapplistxmlurl = null; // remote ldschema
	static java.net.URL ldschemaxmlurl = null; // local applist
	...
	static String serverName; // use for store server name
	...
	public MainAppList(Hashtable s_ht,String baseURL_h,int port_h) {
		...
		serverName = PropertyLoader.getConfig("dp.dns.name");
	...
	public Vector constructApplist(String applist_url, String s_user_for) throws Exception{
		...
		try{
			...
			for (int i=0; i<appNodeList.getLength(); i++) {
		        ...
		        if (tempAppNode.getNodeName().equalsIgnoreCase(Constant.appElement)) {
					if (tempAppNode.hasChildNodes()) {
						...
						logger.debug(s_user_for.toUpperCase()+" Applistion["+applicationCount+"] elements Loading Start");		  		       
						...
						if (canDisplay) {  //ONLY DISPLAY while the condition is passed 
                        	logger.debug("Application["+(String)ht_applist.get(Constant.descElement)+"] can be diplay."); 
                        	...
                        	String str = null;						    	
						   	...
						   	//Only display the link if the link is link to another dept <<old checking have issued - indexOf("dp."+PropertyLoader.getConfig("dp.dept.id")+".ccgo.hksarg")>>
						    	if (((String)ht_applist.get(Constant.urlElement)).indexOf(PropertyLoader.getConfig("dp.dns.name")) == -1||PROTECT_EXT.equalsIgnoreCase("true") ){				
								//**** OSDP's Solution	//
								if ("OSDP".equalsIgnoreCase(solution_provider)) {
									if (saml.equalsIgnoreCase(Constant.saml_1))
										...
										try{
											java.net.URL osdpmappingxmlurl = new java.net.URL(PropertyLoader.getConfig("osdpmapping.xml.url"));
											...
											org.w3c.dom.NodeList childMappingNodeList;
										   	...
										   	//Default entityID would be error
										   	ht_applist.put(Constant.entityIDElement, Constant.defaultEntityID);
										   	
											//Read 1st Level NodeList
											for (int j=0; j<mappingNodeList.getLength(); j++) {
												tempMappingNode = mappingNodeList.item(j);				
												//Handle only service provider Element
										        if (tempMappingNode.getNodeName().equalsIgnoreCase(Constant.spElement)) {
													//Find the matched sp for mapping
										        	if (tempMappingNode.hasChildNodes() && 
															tempMappingNode.getFirstChild().getNodeValue().equalsIgnoreCase((String)ht_applist.get(Constant.abbrElement))) {
														//Read 2st Level NodeList
										                childMappingNodeList = tempMappingNode.getChildNodes();					
										 		        for (int k=0; k<childMappingNodeList.getLength(); k++) {
										                    ...
										                    if (tempChildMappingNode.getNodeName().equalsIgnoreCase(Constant.entityIDElement))
										                    	ht_applist.put(Constant.entityIDElement, tempChildMappingNode.getFirstChild().getNodeValue());
										                    }
										        	}
										        }
											}
										...
										if ("CSP".equalsIgnoreCase((String)ht_applist.get(Constant.abbrElement))){
										//specific format for OSDP solution to CSP -L385 & L391 modified with PropertyLoader.getConfig("idp.dept")
										String target=(String)ht_applist.get(Constant.destination);
										if ("GOVHRMS_CENTRAL".equalsIgnoreCase(target)){
											target=URLEncoder.encode("https://sso.csp.ccgo.hksarg/sso/idpssoinit?metaAlias=/CSPIDP&spEntityID=GOVHRMS_CENTRAL", "UTF-8");
											}else{
											target=URLEncoder.encode("https://eprocurement.csp.ccgo.hksarg:443/eprocurement", "UTF-8");
											}
										String finalTar=URLEncoder.encode("https://" +serverName + "/sso/idpssoinit?metaAlias=/" + PropertyLoader.getConfig("idp.dept") + "&spEntityID=PRDCSPSP&NameIDFormat=urn%3Aoasis%3Anames%3Atc%3ASAML%3A2.0%3Anameid-format%3Atransient&RelayState="+target, "UTF-8");
																															 
										str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a onclick=\"javascript:launchApp('https://sso.csp.ccgo.hksarg/sso/ssoservice.jsp?TARGET="+ finalTar +"')\">" + (String)ht_applist.get(Constant.descElement) + "</a></td></tr>");
										// str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a onclick=\"javascript:window.open('https://sso.csp.ccgo.hksarg/sso/ssoservice.jsp?TARGET="+ finalTar +"','_blank','left=20,top=20,width=900,height=700,toolbar=no,menubar=no,status=no')\">" + (String)ht_applist.get(Constant.descElement) + "</a></td></tr>");
										//end of CSP link formation																																																																						
										}
										else
											str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a href=\"javascript:launchApp('https://" + serverName + "/sso/idpssoinit?metaAlias=/" + PropertyLoader.getConfig("idp.dept") + "&spEntityID=" + (String)ht_applist.get(Constant.entityIDElement) + "&RelayState="+ ((String)ht_applist.get(Constant.urlElement)) + "&NameIDFormat=" + URLEncoder.encode("urn:oasis:names:tc:SAML:2.0:nameid-format:transient", "UTF-8") + "')\">"+(String)ht_applist.get(Constant.descElement)+"</a></td></tr>");
									}
								}
								/*
								//For interop test
								Object String, temp;
								if ("NAM31".equalsIgnoreCase(solution_provider))
									  //str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a href=\"javascript:launchApp('https://"+serverName + ":8443/nidp/saml/idpsend?PID="+(String)ht_applist.get(Constant.urlElement)+"&TARGET="+(String)ht_applist.get(Constant.urlElement)+"')\">"+(String)ht_applist.get(Constant.descElement)+"</a></td></tr>");
									temp = "";
									temp = (String)ht_applist.get(Coanstant.urlElement);
									if(((String)temp).indexOf("app8")>0 ||((String)temp).indexOf("app10")>0)
										str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a href=\"javascript:launchApp('https://"+serverName + ":8443/nidp/saml/metadata/Less&TARGET="+(String)ht_applist.get(Constant.urlElement)+"</a></td></tr>");	
									else
										str = new String("<tr class='applistRow'><td class='appProvider'>" + (String)ht_applist.get(Constant.abbrElement) + "</td><td class='appName'><a href=\"javascript:launchApp('https://"+serverName + ":8443/nidp/saml/metadata&TARGET="+(String)ht_applist.get(Constant.urlElement)+"</a></td></tr>");
								*/
								
								result.add(str);
							}
								
						}else if ("local".equalsIgnoreCase(s_user_for)) { // validating the local applist	
							/**
							String tmp_request_url = (String) ht.get(Constant.request_url);
							
							if ( tmp_request_url.indexOf(".ccgo.") != -1 ) { 
							  // detected is from external login									 
							  String tmp_str =  ((String)ht_applist.get(Constant.urlElement)).replaceAll(".hksarg",".ccgo.hksarg"); // replace the url
                              str = new String("<tr class='applistRow'><td class='appProvider'><a href=\""+tmp_str+"\">"+(String)ht_applist.get(Constant.descElement)+"</a></td></tr>");									
							  
							}else										
							  str = new String("<tr class='applistRow'><td class='appProvider'><a href=\""+(String)ht_applist.get(Constant.urlElement)+"\">"+(String)ht_applist.get(Constant.descElement)+"</a></td></tr>");									
							*/
							
							String tmp_string_url = ((String)ht_applist.get(Constant.urlElement)).toLowerCase();
							
							if ( tmp_string_url.indexOf("http") != -1 || tmp_string_url.startsWith("http") ) 
							   throw new Exception("xml element called dp_app_url not allow start with or contain the word http ot https.");
							
							
							if ( !tmp_string_url.startsWith("/") ) 
							   throw new Exception("xml element called dp_app_url must start with character / .");
								
							str = new String("<tr class='applistRow'><td class='appLocal'><a href=\"javascript:launchApp('"+(String)ht_applist.get(Constant.urlElement)+"')\">"+(String)ht_applist.get(Constant.descElement)+"</a></td></tr>");									
